...
<body>
    ...
    <script>
        ...
        loginForm.addEventListener("submit", function (e) {
            ...
            fetch('adminlogin.php', { method: 'POST', body: formData })
                .then(response => response.json())
                .then(result => {
                    if (!result.success) {
                        passwordError.textContent = result.errors.password || 'Unable to log in.';
                        return;
                    }
                    window.location.href = result.redirect;
                })
                .catch(() => { passwordError.textContent = 'Unable to connect to the server.'; });
        });
    </script>
</body>
</html>