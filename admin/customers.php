...
<body>
            ...
            <div class="page-content">
                ...
                <div class="page-head">
                    ...
                    <div class="page-search">
                        ...
                        <input type="text" placeholder="Search orders, customer or customer ID..">
                    </div>
                </div>
                ...
                <div class="table-card">
                    <table class="customer-table">
                        <thead>
                            <tr>
                                <th class="col-first">Customer</th>
                                <th>Email</th>
                                <th>Phone Number</th>
                                <th>Orders</th>
                                <th>Loyalty</th>
                        ...
                        <tbody id="customerTableBody">
                            ...
                            <tr data-status="active">
                                ...
                                <td>
                                    <span class="cell-stack">
                                        <span>Dionisioalyssa@gmail.com</span>
                                ...
                                <td>
                                    <span class="cell-stack">
                                        <span class="badge badge-loyalty"><i class="fa-solid fa-star"></i> Member</span>
                                        <span class="loyalty-card-no">Card #BC-10002</span>
                                    </span>
                                </td>
                                <td><span class="status-pill status-active"><i class="fa-solid fa-circle"></i>
                                        Active</span></td>
                                <td class="col-last">
                                    <div class="action-wrap">
                                        <button class="action-btn" aria-label="Row actions"><i
                                                class="fa-solid fa-ellipsis-vertical"></i></button>
                                        <div class="action-menu">
                                            <button class="action-item activate" data-action="activate"><i
                                                    class="fa-solid fa-circle-check"></i> Activate Account</button>
                                            <button class="action-item deactivate" data-action="deactivate"><i
                                                    class="fa-solid fa-circle-minus"></i> Deactivate Account</button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <!-- Row 7 - Inactive / Non-member example -->
                            <tr data-status="inactive">
                                ...
                                <td>
                                    <span class="cell-stack">
                                        <span class="badge badge-not-member">Not a Member</span>
                                        <span class="loyalty-card-no">...</span>
                                    </span>
                                </td>
                                <td><span class="status-pill status-inactive"><i class="fa-solid fa-circle"></i>
                                        Inactive</span></td>
                                <td class="col-last">
                                    <div class="action-wrap">
                                        <button class="action-btn" aria-label="Row actions"><i
    ...
    <script>
        ...
        // ---- Row action menu (3-dot popup) ----
        function closeAllActionMenus(except) {
            document.querySelectorAll('.action-wrap.open').forEach(wrap => {
                if (wrap !== except) wrap.classList.remove('open');
            });
        }

        function syncActionState(row) {
            const status = row.getAttribute('data-status'); // 'active' | 'inactive'
            const activateBtn = row.querySelector('.action-item.activate');
            const deactivateBtn = row.querySelector('.action-item.deactivate');

            const isActive = status === 'active';
            activateBtn.disabled = isActive;      // can only activate when currently inactive
            deactivateBtn.disabled = !isActive;    // can only deactivate when currently active
        }

        document.querySelectorAll('#customerTableBody tr').forEach(row => {
            syncActionState(row);

            const wrap = row.querySelector('.action-wrap');
            const toggleBtn = wrap.querySelector('.action-btn');
            ...
            wrap.querySelector('.action-item.activate').addEventListener('click', (e) => {
                if (e.currentTarget.disabled) return;
                setRowStatus(row, 'active');
                wrap.classList.remove('open');
            });

            wrap.querySelector('.action-item.deactivate').addEventListener('click', (e) => {
                if (e.currentTarget.disabled) return;
                setRowStatus(row, 'inactive');
                wrap.classList.remove('open');
            });
        });
        ...
        function setRowStatus(row, status) {
            ...
            if (status === 'active') {
                ...
                pill.innerHTML = '<i class="fa-solid fa-circle"></i> Active';
            } else {
                pill.className = 'status-pill status-inactive';
                pill.innerHTML = '<i class="fa-solid fa-circle"></i> Inactive';
            }
            syncActionState(row);
        }
            ...
            const tbody = customerTableBody;
                tbody.innerHTML = result.customers.map(customer => {
                    ...
                    return `<tr data-id="${customer.id}" data-status="${customer.account_status}">
                        ...
                        <td><span class="status-pill status-${customer.account_status}"><i class="fa-solid fa-circle"></i> ${active ? 'Active' : 'Inactive'}</span></td>
                        <td class="col-last"><div class="action-wrap"><button class="action-btn" aria-label="Row actions"><i class="fa-solid fa-ellipsis-vertical"></i></button><div class="action-menu"><button class="action-item activate" data-action="activate">Activate Account</button><button class="action-item deactivate" data-action="deactivate">Deactivate Account</button></div></div></td>
                    </tr>`;
                }).join('');
                tbody.querySelectorAll('tr').forEach(row => {
                    syncActionState(row);
                    const wrap = row.querySelector('.action-wrap');
                    wrap.querySelector('.action-btn').addEventListener('click', e => { e.stopPropagation(); closeAllActionMenus(wrap); wrap.classList.toggle('open'); });
                    wrap.querySelector('.action-item.activate').addEventListener('click', () => setRowStatusPersisted(row, 'active'));
        ...
        function setRowStatusPersisted(row, status) {
            fetch('admin_data_api.php?action=customer_status', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id: row.dataset.id, status })
        ...
        // ---- Export List (formal PDF report, letterhead style for approval) ----
        const exportBtn = document.getElementById('exportBtn');
        const BRAND_MAROON = [105, 39, 39]; // #692727

        function getRowData(row) {
            const name = row.querySelector('.customer-name')?.textContent.trim() || '';
            const joined = row.querySelector('.customer-joined')?.textContent.replace('Joined', '').trim() || '';

            const emailStack = row.children[1];
            const email = emailStack.querySelector('span:first-child')?.textContent.trim() || '';
            const emailVerified = emailStack.querySelector('.badge') ? 'Verified' : 'Unverified';

            const phoneStack = row.children[2];
            const phone = phoneStack.querySelector('span:first-child')?.textContent.trim() || '';
            const phoneVerified = phoneStack.querySelector('.badge') ? 'Verified' : 'Unverified';

            const orders = row.children[3]?.textContent.trim() || '0';
            ...
            return {
                name, joined, email, emailVerified, phone, phoneVerified,
                orders, loyaltyStatus, cardNo, accountStatus
            };
        }

        // Loads the real business logo if reachable; otherwise falls back to a
        // vector-drawn brand mark so the report still looks complete either way.
        ...
        async function generatePdfReport(rows) {
            ...
            doc.text('CUSTOMER REPORT', pageWidth - marginX, 18, { align: 'right' });
            ...
            const statusLabel = { all: 'All Status', active: 'Active only', inactive: 'Inactive only' }[statusVal];
            const loyaltyLabel = 'Customer List';

            doc.setFontSize(9.5);
            doc.setTextColor(60, 60, 60);
            const metaY = 41;
            doc.setFont('helvetica', 'bold');
            doc.text('Branch:', marginX, metaY);
            doc.setFont('helvetica', 'normal');
            doc.text('Sta. Barbara Branch', marginX + 18, metaY);

            doc.setFont('helvetica', 'bold');
            doc.text('Prepared By:', marginX, metaY + 5.5);
            doc.setFont('helvetica', 'normal');
            doc.text('Admin', marginX + 24, metaY + 5.5);

            doc.setFont('helvetica', 'bold');
            doc.text('Filter Applied:', pageWidth - marginX - 65, metaY);
            doc.setFont('helvetica', 'normal');
            doc.text(`${statusLabel} / ${loyaltyLabel}`, pageWidth - marginX - 40, metaY);

            doc.setFont('helvetica', 'bold');
            doc.text('Total Records:', pageWidth - marginX - 65, metaY + 5.5);
            doc.setFont('helvetica', 'normal');
            doc.text(String(rows.length), pageWidth - marginX - 40, metaY + 5.5);
            ...
            doc.autoTable({
                ...
                head: [['Customer', 'Email', 'Phone Number', 'Orders', 'Loyalty', 'Status']],
                body: tableBody,
                styles: {
                    font: 'helvetica',
                    fontSize: 8.5,
                ...
                didParseCell: (data) => {
                    if (data.section === 'body' && data.column.index === 5) {
                        ...
                        data.cell.styles.textColor = isActive ? [30, 156, 78] : [196, 60, 60];
            ...
            doc.save(`customer-report_${dateStamp}.pdf`);
        }
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', async () => {
                ...
                const rowsToExport = visibleRows.length ? visibleRows : allRows;
