...
function ensurePhoneVerification(mysqli $connect): void
{
    if (!tableExists($connect, 'phone_otp')) {
        runSql($connect, "CREATE TABLE IF NOT EXISTS `phone_otp` (
            `id` int NOT NULL AUTO_INCREMENT,
            ...
            `ip` varchar(45) DEFAULT NULL,
            PRIMARY KEY (`id`),
            KEY `idx_phone_otp_user` (`user_id`),
            KEY `idx_phone_otp_status` (`status`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;", 'Created phone_otp table');
    }

    if (tableExists($connect, 'users') && !columnExists($connect, 'users', 'phone_verified')) {
        runSql($connect, "ALTER TABLE `users` ADD COLUMN `phone_verified` tinyint(1) NOT NULL DEFAULT 0 AFTER `phone`", 'Added users.phone_verified');
    }
}
...
function main(): void
{
    ...
    ensurePhoneVerification($connect);
